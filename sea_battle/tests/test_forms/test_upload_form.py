import base64
from io import BytesIO
from django.test import TestCase
from django.core.files.uploadedfile import InMemoryUploadedFile
from sea_battle.forms import UploadForm


TEST_IMAGE = '''

'''.strip()


class SeaBattleUploadFormTestCase(TestCase):
    def test_upload_form_with_valid_data(self):
        image = InMemoryUploadedFile(
            BytesIO(base64.b64decode(TEST_IMAGE)),
            field_name='tempfile',
            name='tempfile.png',
            content_type='image/png',
            size=3212,
            charset='utf-8',
        )
        form = UploadForm(files={'upload': image})
        self.assertTrue(form.is_valid())

    def test_upload_form_with_no_image(self):
        """Проверка работы формы загрузки изображения. Ч.1

        Случай загрузки изображения, когда изображение не было выбрано.
        """
        form = UploadForm(files={'upload': None})
        self.assertFalse(form.is_valid())
        error_message = form.errors.as_data()['__all__'][0].message
        self.assertEqual(error_message, 'No file chosen.')

    def test_upload_form_with_too_big_image(self):
        """Проверка работы формы загрузки изображения. Ч.2

        Случай загрузки изображения, когда изображение слишком много весит.
        """
        image = InMemoryUploadedFile(
            BytesIO(base64.b64decode(TEST_IMAGE)),
            field_name='tempfile',
            name='tempfile.png',
            content_type='image/png',
            size=90000000,
            charset='utf-8',
        )
        form = UploadForm(files={'upload': image})
        self.assertFalse(form.is_valid())
        error_message = form.errors.as_data()['__all__'][0].message
        self.assertEqual(error_message, 'File size should be less than 5.0 MB (current: 85.8 MB).')
